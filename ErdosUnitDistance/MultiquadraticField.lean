...
theorem sqrtTower_zero (c : ℕ → ℚ) : sqrtTower c 0 = ⊥ :=
  TauCeti.Multiquadratic.sqrtTower_zero (K := ℚ) (fun j => Real.sqrt (c j))

theorem sqrtTower_succ (c : ℕ → ℚ) (n : ℕ) :
    sqrtTower c (n + 1)
      = (sqrtTower c n) ⊔ IntermediateField.adjoin ℚ {Real.sqrt (c n)} :=
  TauCeti.Multiquadratic.sqrtTower_succ (K := ℚ) (fun j => Real.sqrt (c j)) n
...
theorem mem_sup_adjoin_sq {K : IntermediateField ℚ ℝ} {x : ℝ} (hx2 : x ^ 2 ∈ K)
    ...
    ∃ a b : ℝ, a ∈ K ∧ b ∈ K ∧ y = a + b * x :=
  (TauCeti.IntermediateField.mem_sup_adjoin_sq hx2).mp hy
...
theorem squareClass_of_sqrt_mem (c : ℕ → ℚ) (hc : ∀ j, 0 < c j) :
    ∀ n, ∀ r : ℚ, 0 < r → (Real.sqrt r : ℝ) ∈ sqrtTower c n →
      ∃ (T : Finset ℕ) (s : ℚ), ↑T ⊆ Set.Iio n ∧
        r = s ^ 2 * ∏ j ∈ T, c j :=
  fun n r hr hmem =>
    TauCeti.Multiquadratic.squareClass_of_sqrt_mem c (fun j => (hc j).le) n r hr.le hmem
